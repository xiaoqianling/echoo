...
export const PushTestPage = () => {
  ...
  const [useCustomToken, setUseCustomToken] = createSignal(false);
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      if (useCustomToken() && customToken()) {
        // 使用自定义Token发送（模拟外部调用）
        const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/echoo/messages/send`, {
          method: 'POST',
          headers: {
        ...
        // 使用当前登录用户发送
        res = await apiService.sendMessage(payload);
      }
        ...
        <div class="card">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Show when={useCustomToken()}>
              <div class="form-group">
                <label>API Token (以 echoo_sk_ 开头)</label>
                <input
                  type="text"
                  value={customToken()}
                  onInput={(e) => setCustomToken(e.currentTarget.value)}
                  placeholder="echoo_sk_..."
                  required={useCustomToken()}
                />
                <small class="hint">请从 API Tokens 页面生成并复制 Token</small>
              </div>
            </Show>
